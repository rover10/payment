...
func (client *Client) PaymentHistory(account_id int, offset int64, limit int64) ([]model.PaymentHistory, error) {
	...
	rows, err := client.db.Query(`
		SELECT 
			...
			(SELECT left(ua.account_number, -5) || '*****' as sender_acc_no from users_account ua2 INNER JOIN transaction t ON ua2.id = t.from_account_id INNER JOIN bank b ON b.id = ua2.bank_id WHERE ta.id = t.id),
			...
			(SELECT left(ua.account_number, -5) || '*****' as receiver_acc_no from users_account ua2 INNER JOIN transaction t ON ua2.id = t.to_account_id INNER JOIN bank b ON b.id = ua2.bank_id WHERE ta.id = t.id),
